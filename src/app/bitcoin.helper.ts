import { Payment, payments, ECPair, Network, ECPairInterface, address } from '@psf/bitcoincashjs-lib';
...
export function getUnspent(httpClient: HttpClient, address: string): Observable<Unspent[]> {
	return httpClient.get<Unspent[]>(`${environment.explorer}address/utxo/${address}`);
}

export function getTransaction(httpClient: HttpClient, txid: string): Observable<string> {
	return httpClient.get(`${environment.explorer}rawtransactions/getRawTransaction/${txid}`, { responseType: 'text' });
}

export function broadcast(httpClient: HttpClient, txhex: string): Observable<string> {
	return httpClient.get(`${environment.explorer}rawtransactions/sendRawTransaction/${txhex}`, { responseType: 'text' });
}