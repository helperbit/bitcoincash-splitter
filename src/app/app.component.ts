...
import {
	...
	BackupFileSingle, getUnspent, prepareScripts, evaluteFee, broadcast
} from './bitcoin.helper';
import { environment } from 'src/environments/environment';
import { TransactionBuilder, Transaction } from '@psf/bitcoincashjs-lib';
...
export class AppComponent {
	...
	submitUser() {
		...
		getUnspent(this.httpClient, (this.modelUser.backup as BackupFileSingle).address).subscribe(unspents => {
			...
			const tx = new TransactionBuilder({ network: environment.network });
			Promise.all(unspents.map(u => this.prepareInput(u, this.modelUser.backup, pubkeys, 2))).then(ll => {
				console.log(ll);
				ll.forEach(i => tx.addInput(i));

				const cumulativeValue = unspents.reduce((pv, cv) => pv + cv.amount, 0);

				// getFees(this.httpClient).subscribe(fees => {
				const fee = evaluteFee(32, unspents.length, 1);
